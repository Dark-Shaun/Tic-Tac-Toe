...
function getGameBoard(gameTurns){
  let gameBoard=[...INITIAL_GAME_BOARD.map(interArray=>[...interArray])];
...
function getWinner(gameBoard,players){
  ...
  for(const combination of WINNING_COMBINATIONS){
    ...
    const tirdSquareSymbol=gameBoard[combination[2].row][combination[2].column]
    

    if (
        firstSquareSymbol && firstSquareSymbol==secondSquareSymbol 
        &&  firstSquareSymbol== tirdSquareSymbol
    ){
...
function getActivePlayer(turnsGiven){
  let currentPlayer = 'X'

  if(turnsGiven.length>0 && turnsGiven[0].player === 'X'){
        currentPlayer = 'O'}
